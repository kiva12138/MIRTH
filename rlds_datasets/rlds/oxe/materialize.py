...
def make_oxe_dataset_kwargs(
    ...
    if dataset_kwargs["action_encoding"] is ActionEncoding.EEF_POS:
        # NOTE: Original Open-X datasets with EEF_POS actions use 7-D actions:
        #   6 DoF EEF pose deltas + 1 gripper dimension.
        # The LeRobotKitchen datasets (e.g., level1_a / level1_b) only have 6-D
        # actions (no gripper) in their RLDS specification, so we must avoid
        # creating a 7-D mask for them.
        if dataset_name.startswith("libero"):
            # 7-D EEF + gripper actions; only the last (gripper) dim is absolute
            # and skipped for normalization.
            dataset_kwargs["absolute_action_mask"] = [False] * 6 + [True]
            dataset_kwargs["action_normalization_mask"] = [True] * 6 + [False]
        else:
            # 6-D EEF-only actions; treat all dimensions as relative and
